...
exports.registrarIngreso = async (req, res) => {
  ...
  try {
    ...
    if (tipoVehiculo === 'Bicicleta') {
      if (ident.length !== 10) {
        return res.status(400).json({ message: 'El identificador de bicicleta debe tener exactamente 10 caracteres.' });
      }
    } else {
      if (ident.length !== 6) {
...
// --- REGISTRAR SALIDA (Con cálculo de tarifa) ---
exports.registrarSalida = async (req, res) => {
  ...
  try {
    ...
    // Lógica básica de cobro basada en tu resolución
    let total = 0;
    ...
    await db.query(
      'UPDATE control_i_s SET hora_salida = NOW(), fecha_salida = CURDATE(), total_pagar = ? WHERE id_control_i_s = ?', 
      [total, v.id_control_i_s]
    );
    
    res.json({ message: `Salida registrada. Total a cobrar: $${total.toLocaleString()}` });
  } catch (error) {
